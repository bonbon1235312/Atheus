...
export default async function LoggingPage({ params }: Params) {
  const { guildId } = await params;
  const guild = await requireManagedGuild(guildId);
  const channels = await fetchGuildTextChannels(guildId);
  const cfg = await getFeatureConfigRow(guildId, "logging");
  const channelId = (cfg.config.channelId as string) ?? "";

  async function save(formData: FormData) {
    "use server";
    const ch = String(formData.get("channelId") ?? "");
    if (!ch) await disableFeatureConfigRow(guildId, "logging");
    else await setFeatureConfigRow(guildId, "logging", { channelId: ch }, true);
  ...
  return (
    <main className="section-pad">
      <div className="container-studio max-w-2xl">
        ...
        <p className="kicker mt-6">Logging</p>
        <h1 className="type-display mt-4 text-5xl font-semibold tracking-tight md:text-6xl">Audit log.</h1>
        <p className="mt-5 text-chalk/64">Logs joins, leaves, role changes and nickname changes to this channel.</p>

        <form action={save} className="mt-10 grid gap-5">
          <label className="grid gap-2">
            <span className="text-xs font-bold uppercase tracking-[0.18em] text-blurpleHi">Log channel</span>
