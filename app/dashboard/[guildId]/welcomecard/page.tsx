...
export default async function WelcomeCardPage({ params }: Params) {
  const { guildId } = await params;
  const guild = await requireManagedGuild(guildId);
  const channels = await fetchGuildTextChannels(guildId);
  const cfg = await getFeatureConfigRow(guildId, "welcomecard");
  const channelId = (cfg.config.channelId as string) ?? "";

  async function save(formData: FormData) {
    "use server";
    const ch = String(formData.get("channelId") ?? "");
    if (!ch) await disableFeatureConfigRow(guildId, "welcomecard");
    else await setFeatureConfigRow(guildId, "welcomecard", { channelId: ch }, true);
    revalidatePath(`/dashboard/${guildId}/welcomecard`);
  }
  ...
  return (
    <main className="section-pad">
      <div className="container-studio max-w-2xl">
        ...
        <p className="kicker mt-6">Welcome cards</p>
        <h1 className="type-display mt-4 text-5xl font-semibold tracking-tight md:text-6xl">A designed welcome.</h1>
        <p className="mt-5 text-chalk/64">New members get a generated welcome image. Preview it in Discord with <code>/welcomecard test</code>.</p>

        <form action={save} className="mt-10 grid gap-5">
          <label className="grid gap-2">
            <span className="text-xs font-bold uppercase tracking-[0.18em] text-blurpleHi">Welcome channel</span>
