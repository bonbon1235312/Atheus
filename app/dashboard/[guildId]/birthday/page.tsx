...
export default async function BirthdayPage({ params }: Params) {
  const { guildId } = await params;
  const guild = await requireManagedGuild(guildId);
  const channels = await fetchGuildTextChannels(guildId);
  const cfg = await getFeatureConfigRow(guildId, "birthday");
  const channelId = (cfg.config.channelId as string) ?? "";

  async function save(formData: FormData) {
    "use server";
    const ch = String(formData.get("channelId") ?? "");
    if (!ch) await disableFeatureConfigRow(guildId, "birthday");
    else await setFeatureConfigRow(guildId, "birthday", { channelId: ch }, true);
  ...
  return (
    <main className="section-pad">
      <div className="container-studio max-w-2xl">
        ...
        <p className="kicker mt-6">Birthdays</p>
        <h1 className="type-display mt-4 text-5xl font-semibold tracking-tight md:text-6xl">Celebrate members.</h1>
        <p className="mt-5 text-chalk/64">
          Members set theirs with <code>/birthday set</code>; atheus announces them on the day in this channel.
        </p>

        <form action={save} className="mt-10 grid gap-5">
          <label className="grid gap-2">
            <span className="text-xs font-bold uppercase tracking-[0.18em] text-blurpleHi">Announcement channel</span>
